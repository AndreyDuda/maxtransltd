...
if( $Admin->CheckPermission($_params['access_delete']) )
{
    ...
    if ( !isset($id) ):
        ...
        $delete = mysqli_query($db, "DELETE FROM `".DB_PREFIX."_routs` WHERE page_id = '".$_params['page_id']."' AND elem_id = '".$id."'");
        ...
        while( $elem = mysqli_fetch_assoc($get_elem) ){
            unlink($_SERVER["DOCUMENT_ROOT"].'/public/upload/gallery/'.$elem['image']);
            unlink($_SERVER["DOCUMENT_ROOT"].'/public/upload/gallery/thumb/'.$elem['image']);
        }
