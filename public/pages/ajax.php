...
if ($cleanPost['request'] === 'more_buses') {
    $getBuses = $Db->getAll("SELECT id,image,seats_qty,title_" . $Router->lang . " AS title FROM `" . DB_PREFIX . "_buses` WHERE active = '1' ORDER BY sort DESC LIMIT " . (int)$cleanPost['current'] . " ,6");
    foreach ($getBuses as $k => $bus) {
        ?>
        <div class="bus flex-row gap-30">
            <div class="col-lg-6">
                <div class="bus_img">
                    <img src="/public/upload/buses/<?= $bus['image'] ?>" alt="bus" class="fit_img">
                </div>
            </div>
            <div class="col-lg-6">
                <div class="bus_info">
...
if ($cleanPost['request'] === 'orderBus') {
    ...
    function sendCallbackMail($name, $phone, $message, $date) {
        $to = 'max210183@ukr.net';  //max210183@ukr.net
...
if ($cleanPost['request'] === 'remember_ticket') {
    ...
    $canOrderTicket = true;
    $fromCity = $Db->getOne("SELECT title_".$Router->lang." AS title FROM ".DB_PREFIX."_cities WHERE id = '".(int)$cleanPost['fromCity']."' ");
    $toCity = $Db->getOne("SELECT title_".$Router->lang." AS title FROM ".DB_PREFIX."_cities WHERE id = '".(int)$cleanPost['toCity']."' ");
...
if ($cleanPost['request'] === 'check_OrderTicket') {
    ...
    if (strtotime($date) == strtotime(date('Y-m-d',time()))){
        $checkTicketDeparture = $Db->getOne("SELECT departure_time FROM ".DB_PREFIX."_tours_stops WHERE tour_id = '".(int)$cleanPost['id']."' AND stop_id = '".$from."' ");
        if ($from === $toursDeparture) {
            if (strtotime($checkTicketDeparture['departure_time']) - $toursDepartureClosed < strtotime(date('H:i:s',time()))){
                $canOrderTicket = false;
            }
        } else {
            if (strtotime($checkTicketDeparture['departure_time']) - $toursStopsClosed < strtotime(date('H:i:s',time()))){
                $canOrderTicket = false;
            }
        }
    }
...
if ($cleanPost['request'] === 'clear_session_data') {
    ...
    unset($_SESSION['order']['from']);
    unset($_SESSION['order']['to']);
    unset($_SESSION['order']['date']);
    unset($_SESSION['order']['passengers']);
...
if ($cleanPost['request'] === 'route_details') {
    ...
    <div class="route_details_popup_content_wrapper">
        ...
        <div class="route_details_popup_content">
            <div class="route_details_title route_details_block_title h3_title flex_ac" onclick="toggleInfoBlock(this)">
                <?= $GLOBALS['dictionary']['MSG_MSG_TICKETS_MARSHRUT'] ?>
                <img src="/public/images/common/arrow_down.svg" alt="arrow left">
            </div>
            <div class="route_details_points">
                <?
                $mainInfo = $Db->getOne("SELECT t.departure_time, 
                    c1.title_" . $Router->lang . " AS departure_city, 
                    c2.title_" . $Router->lang . " AS departure_station, 
                    t.arrival_time, 
                    c3.title_" . $Router->lang . " AS arrival_city, 
                    c4.title_" . $Router->lang . " AS arrival_station, 
                    t.price 
                    FROM `" . DB_PREFIX . "_tours` t 
                    LEFT JOIN `" . DB_PREFIX . "_cities` c1 ON t.departure_city = c1.id 
                    LEFT JOIN `" . DB_PREFIX . "_cities` c2 ON t.departure_station = c2.id 
                    LEFT JOIN `" . DB_PREFIX . "_cities` c3 ON t.arrival_city = c3.id 
                    LEFT JOIN `" . DB_PREFIX . "_cities` c4 ON t.arrival_station = c4.id 
                    WHERE t.id = '".(int)$cleanPost['id']."' ");
    ...
    WHERE tt.tour_id = '".(int)$cleanPost['id']."' ");
                ...
                foreach ($getStops as $k => $stop) {
                    $stopTitle = $Db->getOne("SELECT section_id,title_" . $Router->lang . " AS title FROM `" . DB_PREFIX . "_cities` 
                    WHERE id = '" . $stop['stop_id'] . "' ");
                    $stopCity = $Db->getOne("SELECT title_" . $Router->lang . " AS title FROM `" . DB_PREFIX . "_cities` WHERE id = '" . $stopTitle['section_id'] . "' ");
                    ...
                    <div class="route_details_point flex_ac <?if ($k == 0){echo 'active';}?>">
                        ...
                        <div class="route_details_point_title par">
                            <?= $stopCity['title'] . ' ' . $stopTitle['title'] ?><?if ($isTransfer) {
                                echo '<img class="transfer_icon" src="/public/images/transfer_white.svg" alt="Пересадка">' . $GLOBALS['dictionary']['MSG_MSG_SCHEDULE_TRANSFER'] . '';
                            }?>
                            <?if (!$stopPrice || $stopPrice['price'] == 0){?>
                                <?=$GLOBALS['dictionary']['MSG_MSG_TICKETS_NET_POSADKI_/_VYSADKI_PASSAZHIROV']?>
                            <?}?>
                        </div>
                    </div>
                    <?
                    ...
                    <? foreach ($busOptionsArrays as $k => $busOptions) {
                        ...
                        <div class="col-sm-4 col-xs-12">
                            <? foreach ($busOptions as $k => $busOption) {
                                $busOptionTitle = $Db->getOne("SELECT title_" . $Router->lang . " AS title FROM `" . DB_PREFIX . "_buses_options` WHERE id = '" . $busOption['option_id'] . "' "); ?>
            ...
            <div class="route_details_rules">
                <div class="route_details_rules_title route_details_block_title h3_title flex_ac"
                     onclick="toggleInfoBlock(this)">
                    <?= $GLOBALS['dictionary']['MSG_MSG_TICKETS_PRAVILA_POVERNENNYA'] ?>
                    <img src="/public/images/common/arrow_down.svg" alt="arrow left">
                </div>
                <div class="route_details_rules_txt par">
                    <? $returnTxt = $Db->getOne("SELECT text_" . $Router->lang . " AS text FROM `" . DB_PREFIX . "_txt_blocks` WHERE id = '7' ") ?>
                    <?= $returnTxt['text'] ?>
                </div>
            </div>
            <div class="route_details_totals flex_ac">
                <div class="route_details_route_price h2_title">
                    <?= $ticketPrice['price'] . ' ' . $GLOBALS['dictionary']['MSG_MSG_TICKETS_GRN'] ?>
                </div>
    ...
    <div class="route_details_popup_content_wrapper">
        ...
        <div class="route_details_popup_content">
            ...
            <div class="route_details_points">
                ...
                foreach ($getStops as $k => $stop) {
                    $stopTitle = $Db->getOne("SELECT section_id,title_" . $Router->lang . " AS title FROM `" . DB_PREFIX . "_cities` WHERE id = '" . $stop['stop_id'] . "' ");
                    $stopCity = $Db->getOne("SELECT title_" . $Router->lang . " AS title FROM `" . DB_PREFIX . "_cities` WHERE id = '" . $stopTitle['section_id'] . "' ") ?>
                    <div class="route_details_point flex_ac <?if ($k == 0){echo 'active';}?>">
                        <div class="route_details_point_time par">
                            <?= date('H:i', strtotime($stop['arrival_time'])) ?>
...
if ($cleanPost['request'] === 'filter') {
    ...
    if ((int)$cleanPost['arrival_time'] > 0) {
    ...
    if ($cleanPost['departure_station']) {
    ...
    if (is_array($getTickets) && count($getTickets) > 0){?>
        ...
        <div class="ticket_cards_wrapper">
            <?foreach ($getTickets as $k => $ticket) {
                ...
                $arrivalDetails = $Db->getOne("SELECT station.id,station.title_".$Router->lang." AS station,city.title_".$Router->lang." AS city,stop.arrival_time FROM ".DB_PREFIX."_cities station 
                                    LEFT JOIN ".DB_PREFIX."_cities city ON city.id = station.section_id
                                    LEFT JOIN ".DB_PREFIX."_tours_stops stop ON stop.stop_id = station.id AND stop.tour_id = '".$ticket['id']."'
                ...
                $month = $Db->getOne("SELECT title_".$Router->lang." AS title FROM ".DB_PREFIX."_months WHERE id = '".(int)$filterDate[1]."' ");
                            ...
                            <div class="ticket_info">
                                <div class="ticket_info_header flex_ac">
                                    <div class="ticket_info_date_block flex_ac">
                                        <img src="/public/images/common/ticket_calendar.svg"
                                             alt="calendar">
                                ...
                                <div class="ticket_ride_info_block flex-row gap-30">
                                    ...
                                    <div class="col-lg-4 col-sm-6 col-xs-12">
                                        <div class="ticket_ride_arrival ticket_ride_info">
                                            <div class="ticket_ride_time flex_ac">
                                                <img src="/public/images/common/clock.svg"
                                                     alt="clock">
...
if ($cleanPost['request'] == 'return_ticket_popup'){
    ...
    if ($checkClientTicket['client_email'] != $User->email){
        ...
        $month = $Db->getOne("SELECT title_".$Router->lang." AS title FROM `".DB_PREFIX."_months` WHERE id = '".date('m',strtotime($getTicketInfo['tour_date']))."' ");
    } ?>
            ...
            <div class="return_ticket_route_info">
                <div class="return_ticket_route_info_header flex_ac">
                    <div class="return_ticket_date par flex_ac">
                        <img src="/public/images/common/calendar_white.svg" alt="calendar">
                        <?
                    ...
                    <div class="flex-row gap-30">
                        <div class="col-md-4 col-sm-6">
                            <div class="return_ticket_time flex_ac">
                                <img src="/public/images/common/clock_light.svg" alt="clock light">
                        ...
                        <div class="col-md-4 hidden-sm col-xs-12">
                            <div class="return_ticket_logo">
                                <div class="return_ticket_logo_wrapper">
                                    <img src="/public/images/common/ticket_logo_light.svg" alt="ticket logo"
                                         class="fit_img">
                                </div>
                ...
                <div class="row">
                    <select class="return_ticket_reason flex_ac">
                        ...
                        <?$getReturnReasons = $Db->getAll("SELECT id,title_".$Router->lang." AS title FROM ".DB_PREFIX."_return_reasons WHERE active = 1 ORDER BY sort DESC ");
...
if ($cleanPost['request'] === 'return_ticket'){
 ...
 WHERE o.id = '".(int)$cleanPost['id']."' ");
    ...
    if ($upd){
        ...
        $to1 = 'max210183@ukr.net';  // Замените на email администратора max210183@ukr.net
        ...
        <body>
            <div class='img_logo' oncontextmenu='return false;'>
                <a href='https://www.maxtransltd.com'>
                <img src='https://www.maxtransltd.com/public/upload/logos/mailLogo.jpeg' alt='MaxTrans LTD' class='logo'>
                </a>
            </div>
...
if ($cleanPost['request'] === 'route_details_schedule'){
    ...
    $mainInfo = $Db->getAll("SELECT stop.title_".$Router->lang." AS stop_title,stop.google_maps_link,stop_city.title_".$Router->lang." AS stop_city,stop_city.section_id AS stop_country_id,ts.departure_time,ts.arrival_time,ts.stop_id FROM ".DB_PREFIX."_tours_stops ts
    ...
    $ticketPrice = $Db->getOne("SELECT price FROM ".DB_PREFIX."_tours_stops_prices WHERE from_stop = '".(int)$cleanPost['departure']."' AND to_stop = '".(int)$cleanPost['arrival']."' AND tour_id = '".(int)$cleanPost['id']."' ");
    $busOptions = $Db->getAll("SELECT bo.title_".$Router->lang." AS bus_option FROM ".DB_PREFIX."_buses_options_connector boc
    ...
    <div class="schedule_details_content">
        <button class="close_details" onclick="toggleRouteDetailsSchedule('0')">
            <img src="/public/images/common/arrow_left_blue.svg" alt="arrow left">
        </button>
                ...
                <div class="schedule_route_row">
                    ...
                    <a href="<?=$tourStop['google_maps_link']?>" class="schedule_route_stop" target="_blank">
                        <div class="schedule_route_city"><?=$tourStop['stop_city']?><?if ($isTransfer) {
                                echo '<img class="transfer_icon" src="/public/images/transfer.svg" alt="Пересадка">' . $GLOBALS['dictionary']['MSG_MSG_SCHEDULE_TRANSFER'] . '';
                            }?></div>
        ...
        <div class="schedule_route_important_txt">
            ...
            <div class="sr_txt">
                <?$importantTxt = $Db->getOne("SELECT text_".$Router->lang." AS text FROM ".DB_PREFIX."_txt_blocks WHERE id = 11");
        ...
        <div class="sr_services">
            <div class="sr_services_title_row h3_title" onclick="toggleDetailsServices(this)">
                ...
                <img src="/public/images/common/arrow_down.svg" alt="arrow">
            </div>
...
if ($cleanPost['request'] === 'route_price_details'){
    ...
    <div class="schedule_details_content">
        ...
        <div class="route_price_details_table par">
            ...
            $departure = $Db->getOne("SELECT cc.id,cc.title_".$Router->lang." AS station,c.title_".$Router->lang." AS city FROM ".DB_PREFIX."_cities c 
                                LEFT JOIN ".DB_PREFIX."_cities cc ON cc.section_id = c.id
                                WHERE cc.id = '".(int)$cleanPost['departure']."' ");
            $arrival = $Db->getOne("SELECT cc.id,cc.title_".$Router->lang." AS station,c.title_".$Router->lang." AS city FROM ".DB_PREFIX."_cities c 
                                LEFT JOIN ".DB_PREFIX."_cities cc ON cc.section_id = c.id
            ...
            <table class="table m-0 tarif_table">
                ...
                <tr>
                    <th>От/До</th>
                    <?php
                    $getRouteStops = $Db->getAll("SELECT ts.id, ts.stop_id, ts.arrival_time, city.title_" . $Router->lang . " AS city, city.section_id AS city_id, station.title_" . $Router->lang . " AS station 
        ...
        <div class="routes_prices_list">
            ...
            <?foreach ($currentStopsPrices as $k=>$routeStopPrice) {
                $getFromStopData = $Db->getAll("SELECT ts.id,ts.stop_id,ts.arrival_time,city.title_".$Router->lang." AS city,station.title_".$Router->lang." AS station
                FROM ".DB_PREFIX."_tours_stops ts
                LEFT JOIN ".DB_PREFIX."_cities station ON station.id = ts.stop_id
                LEFT JOIN ".DB_PREFIX."_cities city ON city.id = station.section_id
                WHERE ts.tour_id = '".(int)$cleanPost['id']."' AND ts.stop_id = '".$routeStopPrice['from_stop']."' ORDER BY ts.stop_num ");
                $getToStopData = $Db->getAll("SELECT ts.id,ts.stop_id,ts.arrival_time,city.title_".$Router->lang." AS city,station.title_".$Router->lang." AS station
                FROM ".DB_PREFIX."_tours_stops ts
                LEFT JOIN ".DB_PREFIX."_cities station ON station.id = ts.stop_id
                LEFT JOIN ".DB_PREFIX."_cities city ON city.id = station.section_id
...
if ($cleanPost['request'] === 'order_mail') {
    ...
    if ($ticketInfo && $order) {
        ...
        <html>
        <head>
            <title>Інформація про заброньованний квиток:</title>
            <style>
                .email-content {
                    border-left: 4px solid #40A6FF;
                    padding-left: 10px;
        ...
        <body>
            ...
            <p><b>Дякуємо, що обрали нашу компанію!</b> <br>Інформація про заброньованний квиток:</p>
            <div class='email-content'>
                <table>
                    <tr>
                        <td class='email-titles'>Рейс</td>
