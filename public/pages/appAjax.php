...
$cleanPost = json_decode($json, true);
...
if ($cleanPost['request'] === 'AuthCode') {
    ...
    if ($userInfo['code'] === $cleanPost['code']) {
...
if ($cleanPost['request'] === 'routeStations') {
    $getStops = $Db->getAll("SELECT stop_id,arrival_time,departure_time,arrival_day,stop_num,st.title_".$cleanPost['lang']." AS stop_title,st.section_id,ct.title_".$cleanPost['lang']." AS city_title FROM ".DB_PREFIX."_tours_stops 
    LEFT JOIN ".DB_PREFIX."_cities st ON st.id = stop_id 
