...
<head>
    <link rel="stylesheet" href="/public/libs/custom_scroll/jquery.mCustomScrollbar.min.css">
    ...
    END ASC,tsp.price DESC LIMIT " . $pagination['from'] . "," . $pagination['per_page']);
                ...
                <div class="shedule_table_container">
                    <div class="shedule_title h2_title">
                        <? if (!$filterParams) {
                            echo $Router->writetitle(73);
                        } elseif ($filterParams && isset($_GET['country'])) {
                            echo $GLOBALS['dictionary']['MSG_MSG_SCHEDULE_ROZKLAD_NAPRAVLENNYA'] . ' ' . $country['title'];
                        } elseif ($filterParams && isset($_GET['departure']) && isset($_GET['arrival'])) {
                            echo $GLOBALS['dictionary']['MSG_MSG_SCHEDULE_ROZKLAD_NAPRAVLENNYA'] . ' ' . $getRoutes[0]['departure_city_title'] . ' - ' . $getRoutes[0]['arrival_city_title'];
                        } ?>
                    </div>
                    <div class="shedule_table_wrapper">
                        <table class="shedule_table">
                            ...
                            <tr>
                                <th>
                                    <?=$GLOBALS['dictionary']['MSG_MSG_SCHEDULE_KRANA']?>
                                </th>
                                <th><?= $GLOBALS['dictionary']['MSG_MSG_SCHEDULE_REJS'] ?></th>
                                <th><?=$GLOBALS['dictionary']['MSG_MSG_SCHEDULE_MARSHRUT']?></th>
                                <th><?= $GLOBALS['dictionary']['MSG_MSG_SCHEDULE_VARTISTI'] ?></th>
                                <th><?=$GLOBALS['dictionary']['MSG_MSG_SCHEDULE_POSILANNYA_NA_BRONYUVANNYA']?></th>
                            </tr>
                            </thead>
                ...
                WHERE station.station = 1 AND station.section_id = '" . $route['arrival'] . "' AND station.id IN (SELECT stop_id FROM " . DB_PREFIX . "_tours_stops WHERE tour_id = '" . $route['id'] . "')");
                                    ...
                                    <tr class="shedule_tr">
                                        ...
                                        <td class="shedule_td">
                                            <button class="schedule_details_btn" onclick="toggleRouteDetailsSchedule('<?= $route['id'] ?>', '<?= $departureDetails['id'] ?>', '<?= $arrivalDetails['id'] ?>')">
                                                <?= $GLOBALS['dictionary']['MSG_MSG_SCHEDULE_GRAFIK_I_RASPISANIE_REJSA'] ?>
                                            </button>
                                        </td>
                                        <td class="shedule_td h4_title">
                                            ...
                                            <button class="schedule_details_btn" onclick="toggleRoutePricesSchedule('<?= $route['id'] ?>', '<?= $departureDetails['id'] ?>', '<?= $arrivalDetails['id'] ?>', '<?= $nearestDepartureDate ?>')">
                                                <?= $GLOBALS['dictionary']['MSG_MSG_SCHEDULE_PRICE_TABLE'] ?>
                                            </button>
                                        </td>
                                        <td class="shedule_td">
                                            <button class="buy_btn h5_title" onclick="buyTicketFromSchedule(this, '<?= $route['id'] ?>', '<?= $departureDetails['id'] ?>', '<?= $arrivalDetails['id'] ?>', '<?= $nearestDepartureDate ?>')">
                        ...
                        <div class="route_list_block">
                            ...
                            <div class="route_list">
                                ...
                                foreach ($getCountries as $k => $country) {
                                    ?>
                                    <div>
                                        <a href="<?= $Router->writelink(73) ?>?country=<?= $country['id'] ?>"
                        ...
                        <div class="route_list_block">
                            ...
                            <div class="route_list">
                                ...
                                foreach ($getCities as $k => $city) {
                                    ?>
                                    <div>
                                        <a href="<?= $Router->writelink(73) ?>?city=<?= $city['id'] ?>"
...
<script src="/public/libs/custom_scroll/jquery.mCustomScrollbar.js"></script>
<script>
